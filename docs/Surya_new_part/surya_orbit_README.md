...
# 加载模型
ckpt = torch.load("checkpoints/phase1/phase1_epoch20.pt")
model = OrbitHelioSpectFormer(img_size=2048, ..., learned_flow=True)
model.load_state_dict(ckpt["model_state_dict"])
model.eval()

# 同一张 SDO 图，不同距离，看太阳大小是否变化
for dist_au in [0.83, 1.0, 1.5, 2.5, 3.2]:
    batch = make_test_batch(sdo_image, dist_au)  # 你的测试代码
    pred = model(batch)
    # 验证：dist_au 越大，太阳圆面应越小
```
